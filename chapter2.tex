...
\end{equation}
where $m=2a+1$ and $n=2b+1$, $w(s,t)$ is the filter mask, with its center at $w(0,0)$ aligned with the pixel at location $(x,y)$. $g(x,y)$ is the response of the filter at location $(x,y)$. $x$ and $y$ are varied such that each pixel in the image is visited. The observant reader will notice that equation \ref{eq:spatial_filt} corresponds to correlation between the image $f(x,y)$ and the filter mask $w(x,y)$.
